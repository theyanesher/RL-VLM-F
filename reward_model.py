...
#     #     # breakpoint()
#     #     # BCE loss
#     #     # loss = F.binary_cross_entropy(p_t, labels, reduction='mean')
#     #     loss = F.binary_cross_entropy(p_t, labels)
#     #     return loss

#     def regoLoss(self, labels, r_hat_1, r_hat_2, t_t_1, t_t_2):
#         alpha1 = torch.from_numpy(t_t_1/250).to(device)
#         alpha2 = torch.from_numpy(t_t_2/250).to(device)
#         seg1 = torch.sum(alpha1 * r_hat_1, dim=1)  # shape: [batch, 1] or [batch]
#         seg2 = torch.sum(alpha2 * r_hat_2, dim=1)
#         logits = torch.cat([seg1, seg2], dim=1)  # shape: [batch, 2]
#         prob = F.softmax(logits, dim=1)[:, 0]    # P(seg1 > seg2)
...
#     def add_data_batch(self, obses, rewards, tsteps, imgs=None):
...
#             if imgs is not None:
#                 self.img_inputs.append(imgs[index])
...
#                 self.wandb.log({"reward_p_1_1":r_hat1[0], "reward_p_1_2":r_hat1[1], "reward_p_1_3":r_hat1[2]})
#                 r_hat1 = r_hat1.sum(axis=1)
#                 r_hat2 = r_hat2.sum(axis=1)
#                 r_hat = torch.cat([r_hat1, r_hat2], axis=-1)                
...
#     def get_queries(self, mb_size=30): # mb_size - mini-batch size
#         len_traj, max_len = len(self.inputs[0]), len(self.inputs)  # len_traj = 250, max_len = max_size
#         print('len traj, max_len', len_traj, max_len)
...
#         train_timesteps = np.array(self.timesteps[:max_len])
#         if self.vlm_label or self.image_reward:
#             train_images = np.array(self.img_inputs[:max_len])
#             print('img input size', len(self.img_inputs))
...
#         t_t_2 = train_timesteps[batch_index_2]
#         if self.vlm_label or self.image_reward:
...
#         # Generate time index 
#         time_index = np.array([list(range(i*len_traj, i*len_traj+self.size_segment)) for i in range(mb_size)]) # is len of traj same for all ? yes(look at collect_dataset.py)
...
#                 # print('image time index', image_time_index)
#             else:
...
#         sum_r_t_1 = np.sum(r_t_1, axis=1) # taking the sum of the rewards of the size_segment
#         sum_r_t_2 = np.sum(r_t_2, axis=1)
        
#         # skip the query
#         if self.teacher_thres_skip > 0: # set to 0 now
...
#         rand_num = np.random.rand(len_labels) # generates random number within 1 in a list of len len_labels
...
#         self.train_times += 1
#         # print(len(self.inputs), 'inputs shape')
...
#         max_len = self.capacity if self.buffer_full else self.buffer_index
#         total_batch_index = []
#         for _ in range(self.de): # self.de = no of members in ensemble
#             total_batch_index.append(np.random.permutation(max_len)) # shuffles the batch
        
#         num_epochs = int(np.ceil(max_len/self.train_batch_size)) # train batch_size 64
#         total = 0
...
def KCenterGreedy(obs, full_obs, num_new_sample):
    ...
    for count in range(num_new_sample):
        ...
        new_obs = obs[current_index]
        if isinstance(full_obs, torch.Tensor):
            new_full_obs = torch.cat([
                full_obs, 
                obs[selected_index]], 
                dim=0)
        else:
            ...
            if full_start < len(obs):
                ...
                for idx in range(len(full_obs) // batch_size + 1):
                    ...
                    if start < len(full_obs):
                        ...
                        dist = torch.norm(
                            obs[full_start:full_end, None, :] - full_obs[None, start:end, :], dim=-1, p=2
                        )
                        dists.append(dist)
                dists = torch.cat(dists, dim=1)
...
class RewardModel:
    def __init__(self, ds, da, 
        ...
        # CONVERT ALL BUFFERS TO GPU TENSORS
        if not image_reward:
            self.buffer_seg1 = torch.empty((self.capacity, size_segment, self.ds+self.da), 
                                          dtype=torch.float32, device=device)
            self.buffer_seg2 = torch.empty((self.capacity, size_segment, self.ds+self.da), 
                                          dtype=torch.float32, device=device)
        else:
            assert self.size_segment == 1
            self.buffer_seg1 = torch.empty((self.capacity, 1, image_height, image_width, 3), 
                                          dtype=torch.uint8, device=device)
            self.buffer_seg2 = torch.empty((self.capacity, 1, image_height, image_width, 3), 
                                          dtype=torch.uint8, device=device)
            self.image_height = image_height
            self.image_width = image_width
            self.resize_factor = resize_factor

        self.buffer_label = torch.empty((self.capacity, 1), dtype=torch.float32, device=device)
        ...
        # Convert data storage to GPU tensors for faster access
        self.inputs = []  # Keep as list for dynamic sizing, convert elements to tensors on GPU
        self.targets = []
        self.raw_actions = []
        self.img_inputs = []
        self.timesteps = []
    ...
    #     if not isinstance(t_t_1, torch.Tensor):
    #         alpha1 = torch.from_numpy(t_t_1/250).to(device)
    #     else:
    #         alpha1 = (t_t_1/250).to(device)
            
    #     if not isinstance(t_t_2, torch.Tensor):
    #         alpha2 = torch.from_numpy(t_t_2/250).to(device)
    #     else:
    #         alpha2 = (t_t_2/250).to(device)
    ...
    def add_data(self, obs, act, rew, done, img=None):
        # Convert numpy arrays to tensors on GPU immediately
        sa_t = np.concatenate([obs, act], axis=-1)
        r_t = rew

        flat_input = torch.from_numpy(sa_t.reshape(1, self.da+self.ds)).float().to(device)
        r_t = np.array(r_t)
        flat_target = torch.from_numpy(r_t.reshape(1, 1)).float().to(device)
        
        if img is not None:
            flat_img = torch.from_numpy(img.reshape(1, img.shape[0], img.shape[1], img.shape[2])).to(device)
        ...
        if init_data:
            ...
            if 'Cloth' not in self.env_name:
                self.inputs[-1] = torch.cat([self.inputs[-1], flat_input], dim=0)
                self.targets[-1] = torch.cat([self.targets[-1], flat_target], dim=0)
                if img is not None:
                    self.img_inputs[-1] = torch.cat([self.img_inputs[-1], flat_img], dim=0)
                ...
                if len(self.inputs) > self.max_size:
                    ...
                    if img is not None:
                        self.img_inputs = self.img_inputs[1:]
                self.inputs.append(torch.empty(0, self.da+self.ds, device=device))
                self.targets.append(torch.empty(0, 1, device=device))
                if img is not None:
                    self.img_inputs.append(torch.empty(0, img.shape[0], img.shape[1], img.shape[2], device=device))
            else: # clothfold env has is only a 1 step MDP
                self.inputs.append(flat_input)
                self.targets.append(flat_target)
                if img is not None:
    ...
    def add_data_batch(self, obses, rewards, tsteps, imgs=None):
        ...
        for index in range(num_env):
            ...
            # Convert to GPU tensors immediately
            obs_tensor = torch.from_numpy(obses[index]).float().to(device)
            reward_tensor = torch.from_numpy(rewards[index]).float().to(device)
            tstep_tensor = torch.from_numpy(tsteps[index]).float().to(device)
            
            self.inputs.append(obs_tensor)
            self.targets.append(reward_tensor)
            self.timesteps.append(tstep_tensor)
            
            if imgs is not None:
                # img_tensor = torch.from_numpy(imgs[index]).to(device)
                img_i = imgs[index]
                if isinstance(img_i, np.ndarray):
                    img_tensor = torch.from_numpy(img_i).to(device)
                elif isinstance(img_i, torch.Tensor):
                    img_tensor = img_i.to(device)
                else:
                    raise TypeError(f"Unknown type for img_i: {type(img_i)}")
                self.img_inputs.append(img_tensor)
    ...
    def r_hat(self, x):
        ...
        r_hats = []
        for member in range(self.de):
            r_hats.append(self.r_hat_member(x, member=member).detach())
    ...
    def get_train_acc(self):
        ensemble_acc = torch.zeros(self.de, device=device)
        max_len = self.capacity if self.buffer_full else self.buffer_index
        total_batch_index = torch.randperm(max_len, device=device)
        batch_size = 256
        num_epochs = int(np.ceil(max_len/batch_size))
        ...
        for epoch in range(num_epochs):
            ...
            for member in range(self.de):
                ...
                if hasattr(self, 'wandb') and self.wandb:
                    self.wandb.log({"reward_p_1_1":r_hat1[0].item(), "reward_p_1_2":r_hat1[1].item(), "reward_p_1_3":r_hat1[2].item()})
                r_hat1 = r_hat1.sum(axis=1)
                r_hat2 = r_hat2.sum(axis=1)
                r_hat = torch.cat([r_hat1, r_hat2], axis=-1)                
                _, predicted = torch.max(r_hat.data, 1)
        ...
        ensemble_acc = ensemble_acc / total
        return torch.mean(ensemble_acc).item()
    ...
    def get_queries(self, mb_size=30): # mb_size - mini-batch size
        ...
        # Convert list of tensors to stacked tensors for easier manipulation
        train_inputs = torch.stack([inp for inp in self.inputs[:max_len]])  # Shape: [max_len, traj_len, feature_dim]
        train_targets = torch.stack([tgt for tgt in self.targets[:max_len]])  # Shape: [max_len, traj_len, 1]
        train_timesteps = torch.stack([ts for ts in self.timesteps[:max_len]])  # Shape: [max_len, traj_len, 1]
        
        if self.vlm_label or self.image_reward:
            train_images = torch.stack([img for img in self.img_inputs[:max_len]])
            print('img input size', len(self.img_inputs))
            if 'Cloth' in self.env_name:
                train_images = train_images.squeeze(1)

        # Use torch.randint for GPU sampling
        batch_index_2 = torch.randint(0, max_len, (mb_size,), device=device)
        sa_t_2 = train_inputs[batch_index_2] # Batch x T x dim of s&a
        r_t_2 = train_targets[batch_index_2] # Batch x T x 1
        t_t_2 = train_timesteps[batch_index_2]
        if self.vlm_label or self.image_reward:
        ...
        batch_index_1 = torch.randint(0, max_len, (mb_size,), device=device)
        sa_t_1 = train_inputs[batch_index_1] # Batch x T x dim of s&a
        r_t_1 = train_targets[batch_index_1] # Batch x T x 1
        t_t_1 = train_timesteps[batch_index_1] # Batch x T x 1
        if self.vlm_label or self.image_reward:
        ...
        # Generate time index using torch tensors
        time_index = torch.arange(mb_size, device=device).unsqueeze(1) * len_traj + torch.arange(self.size_segment, device=device).unsqueeze(0)
        
        if 'Cloth' not in self.env_name:
            random_idx_2 = torch.randint(0, len_traj-self.size_segment, (mb_size, 1), device=device)
            time_index_2 = time_index + random_idx_2
            random_idx_1 = torch.randint(0, len_traj-self.size_segment, (mb_size, 1), device=device)
            time_index_1 = time_index + random_idx_1
        else:
            time_index_2 = time_index
            time_index_1 = time_index
        ...
        if self.vlm_label or self.image_reward:
            if self.vlm_label == 1 or self.image_reward: # use a single image for querying vlm for the labeling
                image_time_index = torch.arange(mb_size, device=device).unsqueeze(1) * len_traj + (self.size_segment - 1)
            else:
                interval = self.size_segment // self.vlm_label
                image_time_index = torch.arange(mb_size, device=device).unsqueeze(1) * len_traj + (self.size_segment - 1) - torch.arange(self.vlm_label, device=device).unsqueeze(0) * interval
                image_time_index = torch.clamp(image_time_index, min=0)
        ...
        # Use advanced indexing instead of np.take
        sa_t_1 = sa_t_1[time_index_1] # Batch x size_seg x dim of s&a
        r_t_1 = r_t_1[time_index_1] # Batch x size_seg x 1
        t_t_1 = t_t_1[time_index_1] # Batch x size_seg x 1
        sa_t_2 = sa_t_2[time_index_2] # Batch x size_seg x dim of s&a
        r_t_2 = r_t_2[time_index_2] # Batch x size_seg x 1
        t_t_2 = t_t_2[time_index_2] # Batch x size_seg x 1

        if self.vlm_label or self.image_reward:
            img_t_1 = img_t_1[image_time_index_1] # Batch x vlm_label x *img_dim
            img_t_2 = img_t_2[image_time_index_2] # Batch x vlm_label x *img_dim
            
            batch_size, horizon, image_height, image_width, _ = img_t_1.shape

            # Use torch operations instead of numpy
            img_t_1 = img_t_1.permute(0, 2, 1, 3, 4).reshape(batch_size, image_height, horizon * image_width, 3)
            img_t_2 = img_t_2.permute(0, 2, 1, 3, 4).reshape(batch_size, image_height, horizon * image_width, 3)
    ...
    def put_queries(self, sa_t_1, sa_t_2, t_t_1, t_t_2, labels):
        ...
        if not isinstance(sa_t_1, torch.Tensor):
            sa_t_1 = torch.from_numpy(sa_t_1).to(device)
        if not isinstance(sa_t_2, torch.Tensor):
            sa_t_2 = torch.from_numpy(sa_t_2).to(device)
        if not isinstance(t_t_1, torch.Tensor):
            t_t_1 = torch.from_numpy(t_t_1).to(device)
        if not isinstance(t_t_2, torch.Tensor):
            t_t_2 = torch.from_numpy(t_t_2).to(device)
        ...
        if next_index >= self.capacity:
            self.buffer_full = True
            maximum_index = self.capacity - self.buffer_index
            self.buffer_seg1[self.buffer_index:self.capacity].copy_(sa_t_1[:maximum_index])
            self.buffer_seg2[self.buffer_index:self.capacity].copy_(sa_t_2[:maximum_index])
            self.buffer_label[self.buffer_index:self.capacity].copy_(labels[:maximum_index])
            self.buffer_tstep1[self.buffer_index:self.capacity].copy_(t_t_1[:maximum_index])
            self.buffer_tstep2[self.buffer_index:self.capacity].copy_(t_t_2[:maximum_index])

            remain = total_sample - maximum_index
            if remain > 0:
                self.buffer_seg1[0:remain].copy_(sa_t_1[maximum_index:])
                self.buffer_seg2[0:remain].copy_(sa_t_2[maximum_index:])
                self.buffer_label[0:remain].copy_(labels[maximum_index:])
                self.buffer_tstep1[0:remain].copy_(t_t_1[maximum_index:])
                self.buffer_tstep2[0:remain].copy_(t_t_2[maximum_index:])

            self.buffer_index = remain
        else:
            self.buffer_seg1[self.buffer_index:next_index].copy_(sa_t_1)
            self.buffer_seg2[self.buffer_index:next_index].copy_(sa_t_2)
            self.buffer_label[self.buffer_index:next_index].copy_(labels)
            self.buffer_tstep1[self.buffer_index:next_index].copy_(t_t_1)
            self.buffer_tstep2[self.buffer_index:next_index].copy_(t_t_2)
            self.buffer_index = next_index

    # sa_t_1 is list of state-action pairs and r_t_1 is list of ground-truth rewards for these        
    def get_label(self, sa_t_1, sa_t_2, r_t_1, r_t_2, t_t_1, t_t_2, img_t_1=None, img_t_2=None):
        # Convert to numpy for label generation logic (keeping existing logic intact)
        if isinstance(r_t_1, torch.Tensor):
            sum_r_t_1 = r_t_1.sum(dim=1).cpu().numpy() # taking the sum of the rewards of the size_segment
            sum_r_t_2 = r_t_2.sum(dim=1).cpu().numpy()
        else:
            sum_r_t_1 = np.sum(r_t_1, axis=1) # taking the sum of the rewards of the size_segment
            sum_r_t_2 = np.sum(r_t_2, axis=1)
        ...
        if self.teacher_thres_skip > 0: # set to 0 now
            ...
            # apply mask, drop those entries which have very low reward
            if isinstance(sa_t_1, torch.Tensor):
                sa_t_1 = sa_t_1[max_index]
                sa_t_2 = sa_t_2[max_index]
                r_t_1 = r_t_1[max_index]
                r_t_2 = r_t_2[max_index]
                t_t_1 = t_t_1[max_index]
                t_t_2 = t_t_2[max_index]
                sum_r_t_1 = r_t_1.sum(dim=1).cpu().numpy() # updated reward
                sum_r_t_2 = r_t_2.sum(dim=1).cpu().numpy()
            else:
                sa_t_1 = sa_t_1[max_index]
                sa_t_2 = sa_t_2[max_index]
        ...
        # perfectly rational, means not making any mistakes or adding any noise
        if isinstance(r_t_1, torch.Tensor):
            r_t_1_np = r_t_1.cpu().numpy()
            r_t_2_np = r_t_2.cpu().numpy()
        else:
            r_t_1_np = r_t_1
            r_t_2_np = r_t_2
            
        seg_size = r_t_1_np.shape[1]
        temp_r_t_1 = r_t_1_np.copy()
        temp_r_t_2 = r_t_2_np.copy()
        for index in range(seg_size-1):
            temp_r_t_1[:,:index+1] *= self.teacher_gamma
            temp_r_t_2[:,:index+1] *= self.teacher_gamma
        sum_r_t_1 = np.sum(temp_r_t_1, axis=1) # discounted reward sum
    ...
    def kcenter_sampling(self):
        ...
        temp_sa_t_1 = sa_t_1[:,:,:self.ds]
        temp_sa_t_2 = sa_t_2[:,:,:self.ds]
        temp_sa = torch.cat([temp_sa_t_1.reshape(num_init, -1),  
                           temp_sa_t_2.reshape(num_init, -1)], dim=1)
        
        max_len = self.capacity if self.buffer_full else self.buffer_index
        
        tot_sa_1 = self.buffer_seg1[:max_len, :, :self.ds]
        tot_sa_2 = self.buffer_seg2[:max_len, :, :self.ds]
        tot_sa = torch.cat([tot_sa_1.reshape(max_len, -1),  
                          tot_sa_2.reshape(max_len, -1)], dim=1)
        
        selected_index = KCenterGreedy(temp_sa.cpu().numpy(), tot_sa.cpu().numpy(), self.mb_size)
    ...
    def kcenter_disagree_sampling(self):
        ...
        temp_sa = torch.cat([temp_sa_t_1.reshape(num_init_half, -1),  
                           temp_sa_t_2.reshape(num_init_half, -1)], dim=1)
    ...
    def uniform_sampling(self):
        if not self.vlm_label: 
            ...
            if not self.image_reward:
                ...
                # get labels
                sa_t_1, sa_t_2, r_t_1, r_t_2, t_t_1, t_t_2, labels = self.get_label(
                    sa_t_1, sa_t_2, r_t_1, r_t_2, t_t_1, t_t_2)
            else:
                sa_t_1, sa_t_2, r_t_1, r_t_2, t_t_1, t_t_2, img_t_1, img_t_2 =  self.get_queries(
                    mb_size=self.mb_size)
                sa_t_1, sa_t_2, r_t_1, r_t_2, t_t_1, t_t_2, img_t_1, img_t_2, labels = self.get_label(
    ...
    def train_reward(self):
        ...
        total_batch_index = []
        for _ in range(self.de): # self.de = no of members in ensemble
            total_batch_index.append(torch.randperm(max_len, device=device)) # shuffles the batch
        ...
        for epoch in range(num_epochs):
            ...
            for member in range(self.de):
                ...
                if self.image_reward:
                    ...
                    # also we stored uint8 images, we need to convert them to float32
                    sa_t_1 = sa_t_1.float() / 255.0
                    sa_t_2 = sa_t_2.float() / 255.0
                    sa_t_1 = sa_t_1.squeeze(1)
                    sa_t_2 = sa_t_2.squeeze(1)
    ...
    def train_soft_reward(self):
        ...
        total_batch_index = []
        for _ in range(self.de):
            total_batch_index.append(torch.randperm(max_len, device=device))
