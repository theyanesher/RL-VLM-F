...
@torch.no_grad()
def eval_actor(
    env: gym.Env, actor: nn.Module, device: str, n_episodes: int, seed: int, env_name :str, save_gif_dir:str, step:int
) :
    env.seed(seed)
    actor.eval()
    episode_rewards = []
        ...
        if (t + 1) % config.eval_freq == 0:
            ...
            if "metaworld" in config.env:
                eval_scores, success, mean_obj_to_target = eval_actor(
                    ...
                    seed=config.seed,
                    env_name=config.env,
                    save_gif_dir=config.gif_path,
                    step=t
                )
            else:
                eval_scores, success = eval_actor(
                       env,
                    actor,
                    device=config.device,
                    n_episodes=config.n_episodes,
            ...
            if "metaworld" in config.env:
                ...
                wandb.log(
                    {"eval_score": eval_score, "success_percent":success}, step=trainer.total_it
                )
